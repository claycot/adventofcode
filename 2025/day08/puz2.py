...
def main():
    ...
    for link in distance_queue:
        ...
        # if the circuit is now complete, save the connection where it happened
        if len(circuits) == 1:
            break

    ans = -1 
    if i_last_link != -1:
        print(f"connected all boxes after {i_last_link} connections")
        ans = boxes[distance_queue[i_last_link][0]][0] * boxes[distance_queue[i_last_link][1]][0]

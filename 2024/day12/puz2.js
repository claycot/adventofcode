...
fileIter.on('close', function (_) {
    ...
    for (let r = 0; r < map.length; r++) {
        for (let c = 0; c < map[0].length; c++) {
            ...
            // process the island sides
            island.sideCount = countSides(island);
            
            cost += (island.area * island.sideCount);
            islands.push(island);
        }
    }
...
// create an island by lowercasing all cells and getting their area and sides
function mapIsland(map, cell, island) {
    ...
    if (!mapIsland(map, [r - 1, c], island)) {
        if (!island.sides.hasOwnProperty(`t${r}`)) {
            island.sides[`t${r}`] = [];
        }
        island.sides[`t${r}`].push(c);
    }
    // look right
    if (!mapIsland(map, [r, c + 1], island)) {
        if (!island.sides.hasOwnProperty(`r${c + 1}`)) {
            island.sides[`r${c + 1}`] = [];
        }
        island.sides[`r${c + 1}`].push(r);
    }
    // look down
    if (!mapIsland(map, [r + 1, c], island)) {
        if (!island.sides.hasOwnProperty(`b${r + 1}`)) {
            island.sides[`b${r + 1}`] = [];
        }
        island.sides[`b${r + 1}`].push(c);
    }
    // look left
    if (!mapIsland(map, [r, c - 1], island)) {
        if (!island.sides.hasOwnProperty(`l${c}`)) {
            island.sides[`l${c}`] = [];
        }
        island.sides[`l${c}`].push(r);
    }

    return true;
}

function countSides(island) {
    let count = 0;

    Object.values(island.sides).forEach(side => {
        // count each side once
        count++;
        side.sort((a, b) => parseInt(a, 10) - parseInt(b, 10));

        for (let s = 1; s < side.length; s++) {
            // count side again if it breaks mid-stream
            if (side[s] - side[s - 1] !== 1) {
                count++;
            }
        }
